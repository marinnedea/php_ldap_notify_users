...
if($ldapconn) {
	...
	if ($ldapbind) {
		...
		*/ 
		...
		//Set a search filter
		$filter='(&(objectCategory=person)(objectClass=user)(!(sAMAccountName=Guest))(!(sAMAccountName=Administrator))(!(memberOf=CN=ServiceAccounts,OU=Groups,OU=GPSG,DC=gpsg,DC=ibm,DC=ro))(!(memberOf=CN=ToBeDeleted,OU=Groups,OU=GPSG,DC=gpsg,DC=ibm,DC=ro)))';
